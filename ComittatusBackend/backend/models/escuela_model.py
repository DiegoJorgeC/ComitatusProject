...
#Cambie el nombre de la tabla a: escuelas
#Para que no haya confusión :3

class EscuelaModel:
# Corrección en __init__
    def __init__(self):
        self.mysql_pool = MySQLPool()

    def get_escuela(self,idEscuela):
        params = {'idEscuela' : idEscuela}
        rv = self.mysql_pool.execute("SELECT * from escuelas where idEscuela=%(idEscuela)s",params)
        data = []
        content = {}
        for result in rv:
    ...
    def get_all_escuela(self):
        #rv = self.mysql_pool.execute("SELECT * from escuelas where idEscuela=%(idEscuela)s")  
        rv = self.mysql_pool.execute("SELECT * from escuelas")  
        data = []
        content = {}
        for result in rv:
            content = {'idEscuela': result[0], 'escuela': result[1], 'anio': result[2], 'numero_estudiantes': result[3]}
    ...
    def create_escuela(self, idEscuela, escuela, anio, numero_estudiantes):
        ...
        query = """insert into escuelas (idEscuela, escuela,anio,numero_estudiantes)
    ...
    def delete_escuela(self, idEscuela):
        params = {'idEscuela': idEscuela}
        query = """delete from escuelas where idEscuela = %(idEscuela)s"""    
